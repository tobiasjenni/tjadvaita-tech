...
The Sutra reader, timer, and Lila remain live cross-origin applications. Their frames are sandboxed without top-level navigation or form-submission permissions, while retaining scripts, their own origin/storage, source links, and downloads. Backend security remains the responsibility of each original deployment. A frontend cannot keep a secret or enforce API rate limits.
